...
#endif
#if defined(MOZJS_MAJOR_VERSION)
bool js_PluginFacebookJS_PluginFacebook_requestGift(JSContext *cx, uint32_t argc, jsval *vp)
{
    JS::CallArgs args = JS::CallArgsFromVp(argc, vp);
    bool ok = true;
    if (argc == 3) {
    ...
    JS_ReportError(cx, "js_PluginFacebookJS_PluginFacebook_requestGift : wrong number of arguments");
    return false;
}
#elif defined(JS_VERSION)
